...
def get_data():
    ...
    for item in data:
        ...
        ignored_words = ["office", "hour", "hw",
                         "homework", "quiz", "exam", "lecture", "question", "today"]
        filtered_words = [w for w in words if not w.lower(
        ) in stop_words and w.lower() not in ignored_words and w.lower()[:-1] not in ignored_words]
        count = 0
        i = 0
